...
def p_engineKnock(knockingScore):
    if knockingScore == 0:
        return '''
...
def carProblem(prompt, knockingScore):
    global isPetrol
    if prompt == 1:
        if isPetrol:
            return p_engineKnock(knockingScore)
        else:
            return '''
                    Your engine might not have detonation (incorrect air/fuel mixture) but you might hear knocking sounds. As
                    your pistons move up and down in the engine, they turn the crankshaft which ultimately sends power to the
...
def solution(is_petrol, light_check, exp_index, knockingScore):
    global isPetrol
